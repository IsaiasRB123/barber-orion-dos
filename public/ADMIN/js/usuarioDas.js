...
document.getElementById('form-modificar').addEventListener('submit', function(event) {
  ...
  var name = document.getElementById('nombre-modificar').value;
  var email = document.getElementById('correo-modificar').value;
  var tel = document.getElementById('telefono-modificar').value;
  ...
  if (tel.length !== 10) {
    Swal.fire({
      title: '¡Error!',
      text: 'El telefono debe teners 10 caracteres.',
      icon: 'error',
      confirmButtonText: 'Entendido'
    });
    return;
...
document.getElementById('alert-eliminar').addEventListener('click', function() {
    ...
    window.location.href = '/Usuarios'; // Cambia "otra_pagina.html" por la URL de tu página destino
  });
});
